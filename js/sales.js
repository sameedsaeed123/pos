    ...
    .then((data) => {
      if (data.success) {
        ...
        data.accounts.forEach((account) => {
          ...
          option.textContent = `${account.name} (Balance: PKR ${balance.toFixed(2)})`
          option.dataset.balance = balance

          accountSelect.appendChild(option)
        })
      } else {
        showNotification(data.message || "Error loading accounts", "error")
      }
    })
    .catch((error) => {
      console.error("Error loading accounts:", error)
      showNotification("Error loading accounts. Please try again.", "error")
    })
}
...
function updatePaymentOptions() {
  ...
  if (!accountSelect || !paymentMethodSelect) return

  const selectedAccount = accountSelect.value

  if (selectedAccount) {
    // If account is selected, set payment method to "account"
    paymentMethodSelect.value = "account"

    // Disable payment method selection
    paymentMethodSelect.disabled = true
  } else {
    // If no account is selected, enable payment method selection
...
function completeSale() {
  ...
  const paymentStatus = "paid" // Default to paid since we don't have a payment status dropdown
  ...
  fetch("api/process_sale_with_account.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        // Reload accounts dropdown to reflect updated balances
        if (accountId) {
          loadAccountsDropdown()
        }
...
function showReceipt(saleId, transactionId, customerName, saleData) {
  ...
  saleData.items.forEach((item) => {
    itemsHtml += `
      <div class="receipt-item">
          ...
          <div class="receipt-item-price">${item.total_price.toFixed(2)}</div>
      </div>
    `
  })
  ...
  // Add account information if applicable
  let accountInfo = ""
  if (saleData.account_id) {
    const accountSelect = document.getElementById("account-select")
    const selectedOption = accountSelect.options[accountSelect.selectedIndex]
    const accountName = selectedOption.textContent.split(" (")[0]
    accountInfo = `<p><strong>Account:</strong> ${accountName}</p>`
  }
  ...
  receiptContainer.innerHTML = `
    ...
    <div class="receipt-customer">
        ...
        ${accountInfo}
        <p><strong>Payment:</strong> ${saleData.payment_method}</p>
    </div>
    ...
    <div class="receipt-summary">
        ...
        <div class="receipt-total">
            <div>Total:</div>
            <div>${saleData.final_amount.toFixed(2)}</div>
        </div>
    </div>
