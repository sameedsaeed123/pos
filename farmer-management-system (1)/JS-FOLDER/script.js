document.addEventListener("DOMContentLoaded", () => {
  ...
  // Add code to check if we need to refresh account details
  if (window.location.href.includes("account-details.php")) {
    // Check if we came from an update or delete operation
    if (sessionStorage.getItem("entryUpdated") === "true" || sessionStorage.getItem("entryDeleted") === "true") {
      // Clear the flags
      sessionStorage.removeItem("entryUpdated")
      sessionStorage.removeItem("entryDeleted")
...
// Modify the updateEntry function to include a refresh
  ...
  fetch("actions/update_entry.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        sessionStorage.setItem("entryUpdated", "true")
      } else {
...
// Modify the deleteEntry function to include a refresh
  ...
  fetch("actions/delete_entry.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        sessionStorage.setItem("entryDeleted", "true")
      } else {
        showMessage("error", data.message)
      }
    })
...
// Add this function after the deleteEntry function
window.refreshAccountDetails = () => {
  // If we're on the account details page, refresh it
  if (window.location.href.includes("account-details.php")) {
    window.location.reload()
  }
}
