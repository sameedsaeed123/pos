document.addEventListener("DOMContentLoaded", () => {
  ...
  // Form submissions
  document.getElementById("add-mandi-form")?.addEventListener("submit", (e) => {
    e.preventDefault()
    addEntry()
  })

  document.getElementById("edit-mandi-form")?.addEventListener("submit", (e) => {
    e.preventDefault()
    updateEntry()
  })
  ...
  document.getElementById("mandi-search")?.addEventListener("input", (e) => {
  ...
  // Add code to check if we need to refresh mandi details
  if (window.location.href.includes("mandi-details.php")) {
    // Check if we came from an update or delete operation
    if (
      sessionStorage.getItem("mandiEntryUpdated") === "true" ||
      sessionStorage.getItem("mandiEntryDeleted") === "true"
    ) {
      // Clear the flags
      sessionStorage.removeItem("mandiEntryUpdated")
      sessionStorage.removeItem("mandiEntryDeleted")
...
function loadAccounts(search = "", filter = "all") {
  fetch(`actions/get_mandi_accounts.php?search=${encodeURIComponent(search)}`)
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      ...
      if (!data || data.length === 0) {
        tbody.innerHTML = `
          <tr>
            <td colspan="4" class="text-center">No mandi accounts found</td>
          </tr>
        `
        return
      }
      ...
      filteredData.forEach((account) => {
        ...
        row.innerHTML = `
          ...
          <td>
            <a href="mandi-details.php?id=${account.id}" class="action-btn">View Details</a>
          </td>
        `
    ...
    .catch((error) => {
      ...
      tbody.innerHTML = `
        <tr>
          <td colspan="4" class="text-center">Error loading mandi accounts. Please try again.</td>
        </tr>
      `
    })
}

// Find the loadEntries function and ensure it includes the delete button with the correct onclick handler

// Function to load entries
function loadEntries(search = "") {
  fetch(`actions/get_mandi_entries.php?search=${encodeURIComponent(search)}`)
    .then((response) => response.json())
    .then((data) => {
      const tbody = document.getElementById("mandi-list-body")
      tbody.innerHTML = ""

      if (data.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="6" class="text-center">No mandi entries found</td>
            </tr>
        `
        return
      }
        ...
        row.innerHTML = `
            ...
            <td>
                <div class="action-buttons">
                    ...
                    <button class="action-btn delete-btn" onclick="deleteMandiEntry(${entry.id})">Delete</button>
                </div>
            </td>
        `
        tbody.appendChild(row)
    ...
    .catch((error) => {
      ...
      showMessage("error", "Error loading mandi entries")
    })
}
...
window.editEntry = (id) => {
  currentEntryId = id
  fetch(`actions/get_mandi_entry.php?id=${id}`)
    .then((response) => response.json())
    .then((entry) => {
      document.getElementById("edit-mandi-name").value = entry.name
    ...
    .catch((error) => {
      ...
      showMessage("error", "Error loading mandi entry data")
    })
}

function updateEntry() {
  const formData = new FormData(document.getElementById("edit-mandi-form"))
  ...
  fetch("actions/update_mandi_entry.php", {
    method: "POST",
    body: formData,
  })
    .then((response) => {
    ...
    .then((data) => {
      if (data.success) {
        ...
        // Set a flag in sessionStorage to indicate an update happened
        sessionStorage.setItem("mandiEntryUpdated", "true")
      } else {
...
window.deleteMandiEntry = (id) => {
  if (!confirm("Are you sure you want to delete this mandi entry? This action cannot be undone.")) {
    return
  }
  ...
  fetch("actions/delete_mandi_entry.php", {
    method: "POST",
    body: formData,
  })
    .then((response) => {
    ...
    .then((data) => {
      if (data.success) {
        ...
        // Set a flag in sessionStorage to indicate a deletion happened
        sessionStorage.setItem("mandiEntryDeleted", "true")
      } else {
        showMessage("error", data.message)
      }
    })
...
// Add this function after the deleteMandiEntry function
window.refreshMandiDetails = () => {
  // If we're on the mandi details page, refresh it
  if (window.location.href.includes("mandi-details.php")) {
    window.location.reload()
  }
}
...
// Add entry function
function addEntry() {
  const form = document.getElementById("add-mandi-form")
  const formData = new FormData()
  ...
  if (!name) {
    showMessage("error", "Please enter or select a mandi name")
    return
  }
  ...
  // Send request
  fetch("actions/add_mandi_entry.php", {
    method: "POST",
    body: formData,
  })
    .then((response) => {
    ...
    .catch((error) => {
      ...
      showMessage("error", "Failed to add mandi entry: " + error.message)
    })
}
...
function viewAccountDetails(id) {
  ...
  window.location.href = `mandi-details.php?id=${id}`
}
...
function loadNameOptions() {
  const selectMandi = document.getElementById("select-mandi")
  const existingName = document.getElementById("existing-name")

  fetch("actions/get_distinct_mandi_names.php")
    .then((response) => response.json())
    .then((data) => {
      // Update statement form dropdown
      if (selectMandi) {
        selectMandi.innerHTML = '<option value="" disabled selected>Select a mandi</option>'

        if (data.length === 0) {
          const option = document.createElement("option")
          option.value = ""
          option.textContent = "No mandis available"
          option.disabled = true
          selectMandi.appendChild(option)
        } else {
          data.forEach((entry) => {
            const option = document.createElement("option")
      ...
      if (existingName) {
        existingName.innerHTML = '<option value="" disabled selected>Select a mandi</option>'
    ...
    .catch((error) => {
      console.error("Error loading mandi names:", error)
      if (selectMandi) {
        selectMandi.innerHTML = '<option value="" disabled selected>Error loading mandis</option>'
      }
      if (existingName) {
        existingName.innerHTML = '<option value="" disabled selected>Error loading mandis</option>'
      }
    })
}

function generateStatement() {
  const mandiName = document.getElementById("select-mandi").value
  const fromDate = document.getElementById("from-date").value
  const toDate = document.getElementById("to-date").value

  if (!mandiName) {
    showMessage("error", "Please select a mandi")
    return
  }
  ...
  form.method = "POST"
  form.action = "actions/generate_mandi_statement_pdf.php"
  ...
  showMessage("success", "Generating mandi statement...")
}
